...
2. Запустить бота у себя на устройстве:
   ...
   * указать в .env требуемые данные (TOKEN нужно получить у Bot Father, KDEFENDER_API_BASE -- api для webhook/инструкций/`web-api.py`)
     ...
     KDEFENDER_API_BASE=<...>
     ```
   * запустить файлы `k-defender.py` и `web-api.py` (на одном сервере/устройстве)
   * ВАЖНО: для корректной работы верификации подключаемых ботов по webhook необходимо, чтобы файлы запускались на сервере/устройстве с SSL-сертификатом на KDEFENDER_API_BASE (Telegram разрешает использовать только сервера/устройства с https для создания webhook). Если у Вас сервер/устройство с самоподписанным SSL-сертификатом, то необходимо передавать при установке webhook публичный сертификат в формате PEM. Рекомендуется использовать доменное имя с подтверждённым сертификатом (можно подтверждённый на хосте и самоподписанный (например, certbot) на поддомене), тогда не нужно будет указывать публичный сертификат при установке webhook. Подробнее здесь: https://core.telegram.org/bots/webhooks
